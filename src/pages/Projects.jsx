...
import {
  ExternalLink, Github, Star, X, CheckCircle2, Sparkles, Code, Zap, Eye,
  TrendingUp, Heart, Bookmark, Share2, Search, Trophy, Target, Lightbulb,
  AlertCircle, Clock, Users, Activity, BarChart3, Layers, Play, Copy, Check
} from 'lucide-react';
...
const projects = [
  {
    ...
    desc: "AI-powered platform that builds ATS-optimized resumes and improves shortlisting by 2×.",
    longDesc: "Complete full-stack platform using AI to analyze job descriptions, suggest keywords, and generate ATS-friendly PDFs.",
    problem: "Qualified candidates get rejected by ATS filters.",
    solution: "Intelligent resume builder that optimizes formatting and keywords for ATS compatibility.",
    role: "Full ownership — UI/UX, frontend/backend, ATS scoring engine.",
    tech: [
    ...
    impact: ["2× shortlisting", "3k+ resumes", "90%+ ATS score"],
    ...
    duration: "3 months",
    metrics: [
      { label: "Retention", value: "85%", trend: "+12%" },
      { label: "Session", value: "8.5m", trend: "+25%" },
      { label: "Success", value: "92%", trend: "+18%" }
    ]
  },
  {
    id: 2,
    ...
    desc: "Enterprise-grade AI workspace for research, automation, and multi-modal interactions.",
    longDesc: "Unified platform supporting text, images, PDFs, code generation, and advanced prompt engineering.",
    problem: "Professionals juggle disconnected AI tools, losing context.",
    solution: "Unified multi-modal workspace with ultra-fast responses and memory context.",
    role: "Product vision, UI/UX, frontend architecture, performance optimization.",
    tech: [
      { name: "React", icon: "⚛️", color: "#61DAFB" },
    ...
    impact: ["3× faster workflows", "Enterprise UX", "Multi-modal"],
    ...
    duration: "4 months",
    metrics: [
      { label: "Response", value: "<500ms", trend: "-40%" },
  ...
  },
  {
    ...
    desc: "ML-powered career guidance with personalized recommendations based on skills and interests.",
    longDesc: "Intelligent system analyzing student profiles to suggest optimal career trajectories.",
    problem: "Students lack personalized career direction.",
    solution: "Smart ML recommendations with skill gap analysis.",
  ...
  },
  {
    ...
    desc: "Advanced NLP system for real-time fake news detection with explainable AI outputs.",
    longDesc: "End-to-end ML pipeline using TF-IDF + LSTM with 95% accuracy and reasoning.",
    problem: "Fake news spreads misinformation rapidly.",
    solution: "High-accuracy NLP model with explainable real-time detection.",
    role: "Complete ML pipeline: data, preprocessing, training, deployment.",
    tech: [
      { name: "Python", icon: "🐍", color: "#3776AB" },
    ...
    impact: ["95% accuracy", "1M+ articles", "Transparent AI"],
    stats: { accuracy: "95%", articles: "1M+", time: "<100ms" },
    tags: ["NLP", "AI Safety", "Deep Learning"],
    category: "Machine Learning",
    icon: "🛡️",
    ...
    duration: "5 months",
    metrics: [
      { label: "Speed", value: "95ms", trend: "-35%" },
  ...
  },
  {
    ...
    desc: "ML web app predicting heart disease risk with 87% accuracy using clinical data.",
    longDesc: "Complete pipeline and Flask app taking patient parameters to predict disease probability.",
    problem: "Early heart disease detection is often delayed.",
    solution: "User-friendly ML platform trained on clinical datasets.",
    role: "End-to-end: preprocessing, training, Flask backend, frontend.",
    tech: [
      { name: "Python", icon: "🐍", color: "#3776AB" },
      { name: "Scikit-learn", icon: "📊", color: "#F7931E" },
      { name: "Flask", icon: "🔥", color: "#000000" },
    ...
    const observer = new IntersectionObserver(
      entries => entries.forEach(entry => {
        if (entry.isIntersecting) {
          const idx = projectRefs.current.indexOf(entry.target);
          if (idx !== -1) setVisibleProjects(prev => new Set([...prev, idx]));
        }
      }),
    ...
    <>
      <style>{`
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .card:hover { transform: translateY(-8px) scale(1.01); box-shadow: 0 20px 40px rgba(102, 126, 234, 0.3); }
        ...
        .tech-badge { transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); }
      ...
      <div style={{ minHeight: '100vh', background: 'radial-gradient(ellipse at top, #0f0f23 0%, #000 100%)', position: 'relative', padding: '4rem 2rem', overflow: 'hidden' }}>
        <canvas ref={canvasRef} style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', pointerEvents: 'none', zIndex: 1, opacity: 0.6 }} />

        {/* Background Orbs */}
        <div style={{ position: 'absolute', top: '10%', left: '5%', width: '600px', height: '600px', background: 'radial-gradient(circle, rgba(102, 126, 234, 0.3), transparent 70%)', borderRadius: '50%', filter: 'blur(80px)', animation: 'scaleBreath 8s ease-in-out infinite', pointerEvents: 'none', zIndex: 2 }} />
        <div style={{ position: 'absolute', top: '50%', right: '10%', width: '700px', height: '700px', background: 'radial-gradient(circle, rgba(168, 85, 247, 0.25), transparent 70%)', borderRadius: '50%', filter: 'blur(90px)', animation: 'scaleBreath 10s ease-in-out infinite reverse', pointerEvents: 'none', zIndex: 2 }} />
        <div style={{ position: 'absolute', bottom: '5%', left: '40%', width: '550px', height: '550px', background: 'radial-gradient(circle, rgba(0, 245, 255, 0.2), transparent 70%)', borderRadius: '50%', filter: 'blur(75px)', animation: 'scaleBreath 12s ease-in-out infinite', pointerEvents: 'none', zIndex: 2 }} />

        {/* Rotating Circles */}
        <div style={{ position: 'absolute', top: '15%', right: '15%', width: '300px', height: '300px', border: '3px solid rgba(102, 126, 234, 0.3)', borderRadius: '50%', animation: 'rotate 25s linear infinite', pointerEvents: 'none', zIndex: 3 }}>
          <div style={{ position: 'absolute', top: '50%', left: '50%', width: '220px', height: '220px', border: '2px solid rgba(168, 85, 247, 0.25)', borderRadius: '50%', transform: 'translate(-50%, -50%)', animation: 'rotate 18s linear infinite reverse' }} />
        </div>
        <div style={{ position: 'absolute', bottom: '20%', left: '10%', width: '280px', height: '280px', border: '3px solid rgba(168, 85, 247, 0.25)', borderRadius: '50%', animation: 'rotate 30s linear infinite reverse', pointerEvents: 'none', zIndex: 3 }} />
        
        {/* Floating Shapes */}
        <div style={{ position: 'absolute', top: '40%', right: '25%', width: '180px', height: '180px', border: '2px solid rgba(236, 72, 153, 0.25)', borderRadius: '50%', animation: 'floatSlow 14s ease-in-out infinite, rotate 22s linear infinite', pointerEvents: 'none', zIndex: 3 }} />
        <div style={{ position: 'absolute', top: '22%', left: '25%', width: '20px', height: '20px', background: 'rgba(102, 126, 234, 0.7)', borderRadius: '50%', boxShadow: '0 0 25px rgba(102, 126, 234, 0.9)', animation: 'orbitSlow 30s linear infinite', pointerEvents: 'none', zIndex: 4 }} />

        {/* Rectangles */}
        <div style={{ position: 'absolute', top: '12%', left: '65%', width: '250px', height: '180px', border: '3px solid rgba(102, 126, 234, 0.25)', borderRadius: '30px', animation: 'rotate 35s linear infinite, floatSlow 18s ease-in-out infinite', pointerEvents: 'none', zIndex: 3 }} />
        <div style={{ position: 'absolute', bottom: '18%', right: '5%', width: '220px', height: '200px', border: '3px solid rgba(168, 85, 247, 0.25)', borderRadius: '25px', animation: 'rotate 40s linear infinite reverse', pointerEvents: 'none', zIndex: 3 }} />
          ...
          <div className="grid-responsive" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gap: '2rem' }}>
            {filtered.map((p, idx) => (
              <div 
                ...
                style={{ borderColor: p.color, cursor: 'pointer', animation: visibleProjects.has(idx) ? `fadeInUp 0.6s ease-out ${idx * 0.1}s` : 'none', opacity: visibleProjects.has(idx) ? 1 : 0 }}
              >
                <div style={{ height: '250px', position: 'relative', overflow: 'hidden' }}>
                  ...
                  {hoveredCard === p.id && p.live && (
                    <div style={{ position: 'absolute', top: '1rem', left: '1rem', animation: 'slideInLeft 0.3s ease-out' }}>
